...
class DatabaseManager:
        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        INSERT INTO agent_logs (level, message, meta, agent_id) 
                        VALUES (%s, %s, %s, %s)
    ...
    def store_agent_interaction(self, agent_id: str, interaction_type: str, 
                               data: Dict[str, Any], parent_id: Optional[str] = None) -> Optional[str]:
            ...
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        ...
                        (agent_id, interaction_type, data, parent_interaction_id)
                        VALUES (%s, %s, %s, %s)
                        RETURNING id
    ...
    def register_agent(self, agent_id: str, agent_type: str, capabilities: Dict[str, Any]) -> bool:
                ...
                with conn.cursor() as cur:
                    cur.execute("""
                        ...
                        DO UPDATE SET capabilities = EXCLUDED.capabilities, 
                                     ...
                                     status = 'active'
                    """, (agent_id, agent_type, json.dumps(capabilities)))
                    conn.commit()
            return True
        except Exception as e:
            ...
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    pool_info = {
                        ...
                        "used_connections": len(self._connection_pool._used)
                    }
